...
"[project]/app/api/broadcast/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        ...
        // Get user's business
        const { data: business, error: businessError } = await supabase.from("businesses").select("*").eq("email", user.email).single();
        ...
        const { data: conversations, error: conversationsError } = await supabase.from("conversations").select("*").eq("business_id", business.id);
        ...
        const messagePromises = conversations.map(async (conversation)=>{
            const messageId = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$uuid$2f$dist$2d$node$2f$v4$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$export__default__as__v4$3e$__["v4"])();
            const dbData = {
                id: messageId,
            ...
            try {
                // Insert message
                const { data: createdMessage, error: msgError } = await supabase.from("messages").insert(dbData).select().single();
                if (msgError) {
                ...
                if (conversation.customer_email) {
                    try {
                        ...
                        await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$email$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["sendMessageNotificationEmail"])(conversation.customer_email, conversation.customer_name || undefined, business.business_name || "Business", message.trim(), chatUrl);
                    } catch (emailError) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c7a94d3f._.js.map